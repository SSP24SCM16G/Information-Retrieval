...
class DocumentIndexer:
    ...
    def load_documents(self):
        ...
        """
        documents = []
        if os.path.exists("indexermain/proceesed_data1.pkl"):
            # Loads the preprocessed documents from the pickle file.
            documents = pickle.load(
                open('indexermain/proceesed_data1.pkl', 'rb'))
        else:
            ...
            for filename in files:
                ...
                with open(filepath, 'r', encoding="ISO-8859-1-8") as f:
            ...
            pickle.dump(documents, open(
                'indexermain/proceesed_data1.pkl', 'wb'))
            progress_bar.close()
    ...
    def search_tfidf(self, query):
        ...
        top_k_indices = similarities.argsort()[0][-self.k:][::-1]
        top_k_scores = similarities[0][top_k_indices]  # Get the similarity scores for top K indices
        
        # Load the articles of the top K similar documents from the JSON files
        #top_k_documents = [json.load(open(os.path.join(self.data_dir, f"page_{i+1}.json"), 'r', encoding='utf8'))['article'] for i in top_k_indices]
        top_k_documents = []
        for i, score in zip(top_k_indices, top_k_scores):
            doc_file = os.path.join(self.data_dir, f"page_{i+1}.json")
            with open(doc_file, 'r', encoding='utf8') as f:
                article_data = json.load(f)['article']
                top_k_documents.append((article_data, score))
    ...
    def search_word2vec(self, query):
        ...
        top_k_documents = [json.load(open(os.path.join(self.data_dir, f"page_{i+1}.json"), 'r', encoding='utf8'))['article'] for i, (doc, sim) in enumerate(similar_documents) if sim >= similar_documents[0][1]]
        
        return top_k_documents


if __name__ == "__main__":
    indexer = DocumentIndexer(r"/Users/gourusamhitha/Downloads/IR_project/crawler/data")
